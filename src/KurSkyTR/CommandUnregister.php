...
class CommandUnregister extends PluginBase
{
    ...
    function onEnable(): void
    {
        ...
        {
            M::getLogger()->info("A not have unregistered command.");
            return;
        }
        foreach (self::$config->get("Commands") as $command)
        {
            if ($cmdMap->getCommand($command) != null)
            {
                ...
                M::getLogger()->error("Command \"" . $command . "\" not found in the server.");
            }
        }
        M::getLogger()->info(self::$count > 0 ? "Total " . self::$count . " command unregistered." : "A not have unregistered command.");
    }
    ...
    public static function removeUnregisterCommandList(string $commandname = ""): void
    {
        ...
        if (in_array($commandname, $list))
        {
            ...
            M::getLogger()->info("This command is not found command list.");
        }
    }
}
